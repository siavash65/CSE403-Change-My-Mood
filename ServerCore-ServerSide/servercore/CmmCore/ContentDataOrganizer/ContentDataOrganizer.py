...
class ContentDataOrganizer():
    ...
    @staticmethod
    def fetchFlickrPhotos(mood_type, num_photos, tags):
        # grab term
        
        # our database
        current_pics = Pictures.objects.all()
        amount_needed = num_photos - len(current_pics)
        
        # grab from flickr

                
        photos =  self.flickr.walk(tag_mode = 'all', tags = 'happy', extras='url_q') 
        urls = list()
        for i in range(amount_needed):
            photo = photos.next()
            mood = Mood.get(mood_type)
            rank = Rank.objects.create(thumbs_up=0, thumbs_down=0)
            rank.save()
            media = Media.objects.create(rank=rank)
            media.save()
            media.moods.add(mood)
            media.save()
            m = Pictures.objects.create(url=photo.get('url_q'), photo_id=photo.get('id'), media=media)
            print m.url
            m.save()
